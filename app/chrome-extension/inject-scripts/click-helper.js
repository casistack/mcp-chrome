...
if (window.__CLICK_HELPER_INITIALIZED__) {
  ...
  async function clickElement(
    ...
    try {
      ...
      // Wait for navigation or modal appearance
      let navigationOccurred = false;
      let modalDetection = null;

      if (waitForNavigation) {
        navigationOccurred = await navigationPromise;
      } else {
        // Check for modal dialogs after click
        await new Promise((resolve) => setTimeout(resolve, 200)); // Brief delay for modal to appear
        modalDetection = await waitForModalChange(2000, true); // Wait up to 2 seconds for modal
      }
